...
int fieldDensity;
int W;
//steps num = width/fieldDensity = 80 steps
int timeStep = 1;
...
void initialize() {
  //float rx = W/2+random(W/4);
  //float ry = W/8+random(W/4);
   float rx = random(W);
  float ry = random(W);
...
void addNew() {
  ...
  float gy = -fy(mx,my);
  println(gx*gradScale(),gy*gradScale());
